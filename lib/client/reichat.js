...
function init() {
	...
	standby = function () {
		...
		if (window.PointerEvent && /Trident/.test(navigator.userAgent) === true && /(Windows NT 6.1)/.test(navigator.userAgent) === false) {
		...
		document.addEventListener('keydown', listener.documentOnKeydown);
		document.addEventListener('keyup', listener.documentOnKeyup);
	};
	...
	listener.canvasOnPointerMove = function (e) {
		...
		var moved = false,
			x = e.pageX + view.canvasContainer.scrollLeft - view.paint.offsetLeft,
			y = e.pageY + view.canvasContainer.scrollTop - view.paint.offsetTop,
			now = stat.pointerLastMoved = Date.now();
	...
	listener.canvasOnPointerDown = function (e) {
		...
		var x = stat.x = e.pageX + view.canvasContainer.scrollLeft - view.paint.offsetLeft,
			y = stat.y = e.pageY + view.canvasContainer.scrollTop - view.paint.offsetTop;
		...
		if (e.pointerType === 'pen' || e.pointerType === 'mouse') {
			if (stat.pressedKeys[0] === 32) {
				listener.scrollStart(e, true);
				return;
			} else if (e.buttons === 2 || (stat.pressedKeys[0] === 17 && stat.pressedKeys.indexOf(18) !== -1)) {
				if (stat.pressedKeys.indexOf(16) === -1) {
					stat.contexts[0].fillRect(x, y, 1, 1);
					
					var i;
					for (i = 0; i < config.layerCount; i++) {
						stat.contexts[0].drawImage(stat.contexts[i + 5].canvas, x, y, 1, 1, x, y, 1, 1);
					}
					
					util.pickColor(stat.contexts[0], x, y);
				} else {
					util.pickColor(stat.contexts[stat.layerNumber + 5], x, y);
				}
				return;
			} else if (e.buttons === 1) {
				if (stat.wacom) {
	...
	util.setScrollbars = function () {
		...
		var viewWidth = view.canvasContainer.clientWidth,
			...
			scrollLeft = view.canvasContainer.scrollLeft,
			scrollTop = view.canvasContainer.scrollTop,
			canvasWidth = view.canvas.scrollWidth * stat.zoom,
			canvasHeight = view.canvas.scrollHeight * stat.zoom,
	...
	util.setTool = function (type, name) {
		...
		var i, l = view.tools.childNodes.length;
		for (i = 0; i < l; i++) {
			view.tools.childNodes[i].removeClassName('selected');
		}
	...
	util.updateLayerPreview = function () {
		...
		var i;
		for (i = 0; i < config.layerCount; i++) {
			view.layerButtons[i].style.backgroundImage = 'url(' + stat.contexts[i + 5].canvas.toDataURL() + ')';
		}
	};
	...
	util.setZoom = function (zoom) {
		...
		view.canvas.style.transform = 'scale(' + zoom + ')';
		view.overlayCanvas.style.transform = 'scale(' + zoom + ')';
