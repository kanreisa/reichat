...
/*global flagrate, io, requestAnimationFrame, brush */
function init() {
	'use strict';
	...
	standby = function () {
		...
		stat.contexts[0].webkitImageSmoothingEnabled = false;
		...
		stat.contexts[1].webkitImageSmoothingEnabled = false;
		...
		var getImgOnload = function (layerNumber) {
			...
			return function () {
				...
				stat.contexts[layerNumber + 4].drawImage(this, 0, 0);
				stat.changed = true;
		...
		if (window.PointerEvent) {
			...
			view.overlayCanvas.addEventListener('pointerout', listener.canvasOnPointerout);
		} else {
			...
			view.overlayCanvas.addEventListener('mouseout', listener.canvasOnPointerout);
		}
		view.overlayCanvas.addEventListener('contextmenu', listener.canvasOnContextmenu);
	};
	...
	tick = function () {
		...
		for (i = 0; i < config.layerCount; i++) {
			if (stat.painting === true && stat.layerNumber === i) {
				...
				stat.contexts[3].drawImage(stat.contexts[i + 4].canvas, 0, 0);
				...
				stat.contexts[0].drawImage(stat.contexts[i + 4].canvas, 0, 0);
			}
		...
		if (moved && stat.painting) {
			...
			if (stat.wacom && parseInt(stat.wacom.pointerType, 10) !== 0) {
				if (stat.pressure.size) {
					size = size * stat.wacom.pressure;
				}
				if (stat.pressure.alpha) {
					alpha = alpha * stat.wacom.pressure;
				}
			} else if (e.pointerType === 'pen') {
				if (stat.pressure.size) {
					size = size * e.pressure;
				}
				if (stat.pressure.alpha) {
					alpha = alpha * e.pressure;
				}
			}
			...
			if (stat.paintedArea.tlX > x - size >> 0) {
				stat.paintedArea.tlX = x - size >> 0;
				if (stat.paintedArea.tlX < 0) {
					stat.paintedArea.tlX = 0;
				}
			}
			if (stat.paintedArea.tlY > y - size >> 0) {
				stat.paintedArea.tlY = y - size >> 0;
				if (stat.paintedArea.tlY < 0) {
					stat.paintedArea.tlY = 0;
				}
			}
			if (stat.paintedArea.brX < Math.ceil(x + size)) {
				stat.paintedArea.brX = Math.ceil(x + size);
				if (stat.paintedArea.brX > stat.w) {
					stat.paintedArea.brX = stat.w;
				}
			}
			if (stat.paintedArea.brY < Math.ceil(y + size)) {
				stat.paintedArea.brY = Math.ceil(y + size);
				if (stat.paintedArea.brY > stat.h) {
					stat.paintedArea.brY = stat.h;
				}
			}
	...
	listener.canvasOnPointerdown = function (e) {
		...
		var x = stat.x = e.offsetX || e.layerX,
			y = stat.y = e.offsetY || e.layerY;
	...
	listener.canvasOnPointerup = function (e) {
		...
		canvas.width = stat.paintedArea.brX - stat.paintedArea.tlX;
		canvas.height = stat.paintedArea.brY - stat.paintedArea.tlY;
		
		// <- current layer buffer
		context.drawImage(
			stat.contexts[stat.layerNumber + 4].canvas,
			stat.paintedArea.tlX,
			stat.paintedArea.tlY,
			canvas.width,
			canvas.height,
		...
		// -> current layer buffer
		stat.contexts[stat.layerNumber + 4].clearRect(stat.paintedArea.tlX, stat.paintedArea.tlY, canvas.width, canvas.height);
		stat.contexts[stat.layerNumber + 4].drawImage(canvas, stat.paintedArea.tlX, stat.paintedArea.tlY);
	...
	listener.canvasOnPointerout = function (e) {
	...
	listener.canvasOnContextmenu = function (e) {
	...
	listener.socketOnPaint = function (paint) {
		...
		var img = new Image();
		img.src = 'data:image/png;base64,' + paint.data;
		img.width = paint.width;
		img.height = paint.height;
		img.onload = function () {
			
			// -> current layer buffer
			stat.contexts[paint.layerNumber + 4].clearRect(paint.x, paint.y, paint.width, paint.height);
			stat.contexts[paint.layerNumber + 4].drawImage(img, paint.x, paint.y);
	...
	util.prepPointers = function () {
		...
		for (i = 0; i < stat.pointers.length; i++) {
			if (typeof map.client[stat.pointers[i].client.uuid] === 'undefined') {
	...
	util.cleanup = function () {
		...
		view.canvas.removeEventListener('pointerout', listener.canvasOnPointerout);
		...
		view.canvas.removeEventListener('mouseout', listener.canvasOnPointerout);
		view.canvas.removeEventListener('contextmenu', listener.canvasOnContextmenu);
	...
	util.setTool = function (toolName) {
		
		var isExists = false;
		
		if (toolName === 'binary-pen') {
			isExists = true;
