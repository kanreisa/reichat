...
function init() {
	...
	var initialize, entry, standby, ready, tick, updatePointer, drl, tie;
	...
	drl = function (e, start) {
		...
		if (start === true) {
			stat.paintedArea.tlX = Math.min(Math.max(0, pointerX - size >> 0), canvasW - 1);
			stat.paintedArea.tlY = Math.min(Math.max(0, pointerY - size >> 0), canvasH - 1);
			stat.paintedArea.brX = Math.min(Math.max(stat.paintedArea.tlX, Math.ceil(pointerX + size)), canvasW);
			stat.paintedArea.brY = Math.min(Math.max(stat.paintedArea.tlY, Math.ceil(pointerY + size)), canvasH);
		} else {
			...
			if (stat.paintedArea.brX < Math.ceil(pointerX + size)) {
				stat.paintedArea.brX = Math.min(Math.max(stat.paintedArea.tlX, Math.ceil(pointerX + size)), canvasW);
			}
			if (stat.paintedArea.brY < Math.ceil(pointerY + size)) {
				stat.paintedArea.brY = Math.min(Math.max(stat.paintedArea.tlY, Math.ceil(pointerY + size)), canvasH);
			}
		}
	};
