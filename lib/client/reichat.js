...
function init() {
	...
	tick = function () {
		...
		stat.contexts[0].clearRect(0, 0, canvasW, canvasH);
		...
		for (i = 0; i < config.layerCount; i++) {
			if (stat.painting === true && stat.layerNumber === i) {
				...
				// <- layer buffer
				stat.contexts[3].drawImage(stat.contexts[i + 5].canvas, 0, 0);
				...
				// canvas <- layer buffer
				stat.contexts[0].drawImage(stat.contexts[i + 5].canvas, 0, 0);
			}
		...
		// overlay
		stat.contexts[1].clearRect(0, 0, canvasW, canvasH);
	...
	listener.canvasOnPointerMove = function (e) {
		...
		if (stat.painting === false && stat.wacom) {
