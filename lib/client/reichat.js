...
function init() {
	...
	standby = function () {
		...
		var craeteLayerButtonOnLeaveHandler = function (i) {
			return function () {
				stat.layerFocus = -1;
				stat.changed = true;
			};
		...
		for (i = config.layerCount - 1; i >= 0; i--) {
			...
			view.layerButtons[i].on('pointerenter', craeteLayerButtonOnEnterHandler(i));
			view.layerButtons[i].on('pointerleave', craeteLayerButtonOnLeaveHandler(i));
		}
	...
	tick = function () {
		...
		if (stat.layerFocus !== -1) {
			stat.contexts[0].globalAlpha = 0.25;
		}
		
		for (i = 0; i < config.layerCount; i++) {
			if (stat.layerFocus === i) {
		...
		if (e.pointerType === 'pen' || e.pointerType === 'mouse') {
			if (stat.pressedKeys.indexOf(32) !== -1) {
				...
				if (stat.pressedKeys.indexOf(16) === -1) {
					...
					var i;
					for (i = 0; i < config.layerCount; i++) {
						tmpContext.drawImage(stat.contexts[i + 5].canvas, x, y, 1, 1, 0, 0, 1, 1);
					}
