...
function init() {
	...
	tick = function () {
		...
		if (stat.painting === true) {
			...
			if (stat.x >= 0 && stat.y >= 0) {
				...
				brush.draw(paintingCompositionContext, stat.x, stat.y, stat.size, stat.alpha, stat.color, true);
	...
	listener.canvasOnPointerMove = function (e) {
		...
		if (moved && stat.painting) {
			...
			if (stat.mode === 'erase' && stat.tool !== 'eraser') {
				size = localStorage.getItem('tool.eraser.size') || 10;
			}
			...
			var point = [x, y, size, alpha];
			util.tie(stat.strokePoints[0], point);
	...
	listener.canvasOnPointerDown = function (e) {
		...
		if (e.pointerType === 'pen' || e.pointerType === 'mouse') {
			if (stat.pressedKeys.indexOf(32) !== -1) {
				...
				if (stat.pressedKeys.indexOf(16) === -1) {
					...
					var i;
					for (i = 0; i < config.layerCount; i++) {
						tmpContext.drawImage(layerContexts[i].canvas, x, y, 1, 1, 0, 0, 1, 1);
					}
					
					util.pickColor(tmpContext, 0, 0);
				} else {
					util.pickColor(layerContexts[stat.layerNumber], x, y);
				}
				return;
			} else if (e.buttons === 1) {
				if (stat.wacom) {
		...
		if (e.buttons === 1 || e.buttons === 32) {
			...
			var context = paintingContext;// painting buffer
			...
			brush.draw(context, x, y, size, alpha, stat.color, stat.mixing);
